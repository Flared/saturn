...
class TestSubInventory(SubInventory):
    ...
    async def next_batch(
        ...
        if source_item.args["a"] == 0:
            items = [
                Item(args={"b": {"c": 7}}, id="7"),
                Item(args={"b": {"c": 8}}, id="8"),
                Item(args={"b": {"c": 9}}, id="9"),
            ]
        elif source_item.args["a"] == 1:
            items = [
                Item(args={"b": {"c": 4}}, id="4"),
                Item(args={"b": {"c": 5}}, id="5"),
                Item(args={"b": {"c": 6}}, id="6"),
            ]
        elif source_item.args["a"] == 2:
            items = [
                Item(args={"b": {"c": 1}}, id="1"),
                Item(args={"b": {"c": 2}}, id="2"),
                Item(args={"b": {"c": 3}}, id="3"),
            ]
...
async def test_joined_sub_inventory() -> None:
    ...
    batch = await alib.list(inventory.iterate(after='{"a": "0", "b": "6"}'))
    assert [(json.loads(i.id), json.loads(i.cursor or ""), i.args) for i in batch] == [
        (
            {"a": "2", "b": "1"},
            {"a": "1", "b": "1"},
    ...
    assert not await alib.list(inventory.iterate(after='{"a": "1", "b": "3"}'))
